...
import { ChevronDown, ChevronUp, Route, MapPin, Ruler, Wallet, Users, BarChart3, Download, ClipboardList } from 'lucide-react';
...
type Tab = 'overview' | 'trips' | 'points' | 'visits';
...
export function ManagerDashboard() {
  ...
  const filteredReports = (dateFrom || dateTo)
    ? reports.filter(r => {
  ...
  const tabs: { key: Tab; label: string; icon: React.ElementType }[] = [
    ...
    { key: 'visits', label: 'Отчеты', icon: ClipboardList },
  ];

  const exportTripsExcel = () => {
    const data = filteredTrips.map(t => ({
      'Специалист': t.worker?.full_name || 'Неизвестный',
      'Дата': format(new Date(t.started_at), 'dd.MM.yyyy HH:mm'),
      'Дистанция (км)': (t.total_distance_m / 1000).toFixed(1),
      'Компенсация (сум)': t.compensation_uzs
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Поездки');
    XLSX.writeFile(wb, `Поездки_${dateFrom || 'all'}_${dateTo || 'all'}.xlsx`);
  };

  const exportPointsExcel = () => {
    const data = displayedPoints.map(p => ({
      'Название': p.name,
      'Адрес': p.address || '',
      'Статус': statusLabels[p.status] || p.status,
      'Заметки': p.notes || ''
    }));
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Точки');
    XLSX.writeFile(wb, `Точки_${dateFrom || 'all'}_${dateTo || 'all'}.xlsx`);
  };
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'trips' && (
          <div className="p-4 space-y-3">
            ...
            >
              <Download size={16} /> Выгрузить в Excel
            </button>
        ...
        {activeTab === 'points' && (
          <div className="p-4 space-y-3">
            ...
            >
              <Download size={16} /> Выгрузить точки (Excel)
            </button>
